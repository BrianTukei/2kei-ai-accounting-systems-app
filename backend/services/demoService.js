...
class DemoService {
   ...
   */
  async sendDemoConfirmation(booking) {
    try {
...
<body>
    ...
    <div class="content">
      ...
      const result = await emailService.sendEmail(
        ...
        subject,
        message,
        `Demo booking confirmed for ${new Date(booking.preferredDate).toLocaleDateString()} at ${booking.preferredTime}`
      );
   ...
   */
  async sendDemoNotification(booking) {
    try {
...
<body>
    ...
    <div class="content">
        <div class="booking-details">
            ...
            <div class="detail-row">
                ...
                <span class="value"><a href="${booking.website}">${booking.website}</a></span>
            </div>
            ` : ''}
            <div class="detail-row">
                <span class="label">Source:</span>
                <span class="value">${booking.source || 'website'}</span>
            </div>
            <div class="detail-row">
                <span class="label">Priority:</span>
                <span class="value priority-${booking.priority || 'medium'}">${(booking.priority || 'medium').toUpperCase()}</span>
            </div>
        </div>
        ...
        <div class="booking-details">
            ...
            <p style="font-style: italic; color: #666;">"${booking.message}"</p>
        </div>
        ` : ''}
        ...
        <div style="background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 6px; padding: 15px; margin: 20px 0;">
            <p style="margin: 0; color: #856404;">
                <strong>⏰ Quick Action:</strong> This booking was just created. Respond within 2 hours to increase conversion chances!
            </p>
        </div>
        
        <div class="footer">
            <p style="font-size: 12px; color: #999;">
                Booking created on ${new Date(booking.createdAt).toLocaleString()}<br>
                IP: ${booking.ipAddress || 'Unknown'} | User Agent: ${booking.userAgent ? booking.userAgent.substring(0, 100) + '...' : 'Unknown'}
            </p>
        </div>
    </div>
</body>
      ...
      const result = await emailService.sendEmail(
        process.env.ADMIN_EMAIL || 'admin@2kei.com',
        subject,
        message,
        `New demo booking from ${booking.name} at ${booking.company}`
      );
    ...
    try {
      ...
      switch (booking.status) {
        case 'confirmed':
          ...
          message = this.generateConfirmedEmail(booking);
          break;
        case 'cancelled':
          subject = 'Demo Cancelled - 2K AI Accounting Systems';
          message = this.generateCancelledEmail(booking);
          break;
        case 'completed':
          subject = 'Thank You - 2K AI Accounting Demo';
          message = this.generateCompletedEmail(booking);
          break;
        default:
   ...
   */
  async sendRescheduleNotification(booking, oldDateTime) {
    try {
...
<body>
    ...
    <div class="content">
        ...
        <div style="text-align: center; margin-top: 30px;">
            <p style="font-size: 12px; color: #666;">
                Best regards,<br>The 2K AI Accounting Team
            </p>
        </div>
    </div>
</body>
      ...
      const result = await emailService.sendEmail(
        ...
        subject,
        message,
        `Demo rescheduled to ${new Date(booking.preferredDate).toLocaleDateString()} at ${booking.preferredTime}`
      );
   ...
   */
  generateConfirmedEmail(booking) {
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
   ...
   */
  generateCancelledEmail(booking) {
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
   ...
   */
  generateCompletedEmail(booking) {
    return `
<!DOCTYPE html>
<html>
<head>
