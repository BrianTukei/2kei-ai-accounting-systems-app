...
const startIdx = fileStr.indexOf('const processFile = useCallback(async (file: File) => {');
const endIdx = fileStr.indexOf(', []);', startIdx) + 6;
...
if (startIdx > -1 && endIdx > -1) {
  const newProcessFile = \const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const mapped = backendRows.map((r) => {
  ...
  fileStr = fileStr.substring(0, startIdx) + newProcessFile + fileStr.substring(endIdx);
  fs.writeFileSync('src/hooks/useBankImport.ts', fileStr);
  console.log('Patched');
} else {
  console.error('Could not find processFile boundaries!');
}
