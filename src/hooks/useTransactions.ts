...
export const useTransactions = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (SUPABASE_ENABLED) {
        try {
          ...
          if (!error && data && Array.isArray(data)) {
            ...
            orgStorage.setJSON(org.id, STORAGE_KEYS.TRANSACTIONS, mapped);
            setIsLoading(false);
            return;
          }
        } catch (err) {
      ...
      const storedTransactions = orgStorage.getJSON<Transaction[]>(org.id, STORAGE_KEYS.TRANSACTIONS, []);
      if (storedTransactions) {
        setTransactions(storedTransactions);
      } else {
        setTransactions([]);
        orgStorage.setJSON(org.id, STORAGE_KEYS.TRANSACTIONS, []);
      }
    ...
    if (SUPABASE_ENABLED) {
      try {
        ...
        const insertObj = {
          ...
          base_currency_amount: enriched.base_currency_amount,
          exchange_rate_used: enriched.exchange_rate_used,
          exchange_rate_date: enriched.exchange_rate_date,
          user_id: user.id,
