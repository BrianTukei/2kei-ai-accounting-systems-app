...
class MobileMoneyService {
   ...
   */
  async sendMoneyMTN(phoneNumber: string, amount: number, reference: string, callbackUrl: string | null = null) {
    try {
      // Get access token
   ...
   */
  async sendMoneyAirtel(phoneNumber: string, amount: number, reference: string, callbackUrl: string | null = null) {
    try {
      // Get access token
   ...
   */
  async processPayment(phoneNumber: string, amount: number, reference: string, preferredProvider: string | null = null) {
    const results: any[] = [];
   ...
   */
  generateReference(prefix: string = '2KEI') {
   ...
   */
  handleFlutterwaveWebhook(data: any) {
   ...
   */
  async sendMoneyFlutterwave(phoneNumber: string, amount: number, reference: string, network: string) {
    try {
